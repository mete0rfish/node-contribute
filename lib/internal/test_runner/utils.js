...
function getCoverageReport(pad, summary, symbol, color, table) {
  ...
  const report = [];
  ArrayPrototypePush(report, `${color}${prefix}start of coverage report\n`);
  ...
  function printCoverageBodyTree(tree, depth = 0) {
    for (const key in tree) {
      if (tree[key].file) {
        ...
        ArrayPrototypePush(report, writeReportLine({
          __proto__: null,
          file: fileName,
          depth,
  ...
  // -------------------------- Coverage Report --------------------------
  if (table) ArrayPrototypePush(report, addTableLine(prefix, tableWidth));
  ...
  ArrayPrototypePush(report, `${prefix}end of coverage report\n`);
  if (color) {
    ArrayPrototypePush(report, white);
  }
  return ArrayPrototypeJoin(report, '');
}
